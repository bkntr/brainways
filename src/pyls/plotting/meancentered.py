...
def _define_vars(results, cond_lvls=None, grp_lvls=None):
    ...
    ul = results.bootres.contrast_ci[..., 0]
    ll = results.bootres.contrast_ci[..., 1]
...
def plot_contrast(
    ...
    **kwargs : key, value mappings
    ...
    abs_err = np.abs(
        [
            df[df.columns[lv + (num_sig * 2)]].values,
            df[df.columns[lv + num_sig]].values,
        ]
        - df[df.columns[lv]].values
    )
    ax.errorbar(
