...
@pytest.fixture
def brainways_project_path_v0_1_7(mock_image_path: ImagePath, tmpdir) -> Path:
    project_path = (
        Path(os.path.realpath(__file__)).parent / "test_projects/v0.1.7/project.bwp"
    )
    tmp_project_dir = Path(tmpdir / "v0.1.7")
    shutil.copytree(project_path.parent, tmp_project_dir)
    rewrite_image_path(project_path=tmp_project_dir, image_path=mock_image_path)
    return tmp_project_dir / "project.bwp"
