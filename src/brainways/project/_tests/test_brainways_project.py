...
def brainways_tmp_subject(
    ...
    brainways_subject = BrainwaysSubject(
        settings=mock_subject_settings, documents=mock_subject_documents
    )
...
def test_open_subject(
    ...
    assert brainways_subject.settings == mock_subject_settings
...
def test_init_brainways_with_wrong_atlas_raises_exception(
    ...
    with pytest.raises(ValueError):
        BrainwaysSubject(settings=mock_subject_settings, documents=[], atlas=mock_atlas)
...
def test_init_subject_with_nonempty_directory(
    mock_subject_settings: ProjectSettings, tmpdir
):
    (Path(tmpdir) / "test").touch()
    with pytest.raises(FileExistsError):
        BrainwaysSubject(settings=mock_subject_settings, subject_path=Path(tmpdir))
